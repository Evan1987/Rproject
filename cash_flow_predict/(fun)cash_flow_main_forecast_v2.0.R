...
# 是对v1.0的详细版本，支持今后简便的参数细化。需要仔细考量各个参数训练的详细时间跨度。
                                  ...
                                  trainSetPath,calendarPath,detail=F)
{
  ...
  # ******************************** 6. brush 用户投资预测**************************************************
  # 6.1 brush用户投资人数比例训练
  brush_type_num_ratio_train<-cash_flow_type_train(item = "type_num_ratio",
                                               user_type = "brush",
                                               trainTimeSpan = brush_type_trainTimeSpan,
                                               trainSetPath = trainSetPath,
                                               calendarPath = calendarPath)
  # 6.2 brush用户单人投资额度训练
  brush_amount_per_num_train<-cash_flow_type_train(item = "amount_per_num",
                                               user_type = "brush",
                                               trainTimeSpan = brush_type_trainTimeSpan,
                                               trainSetPath = trainSetPath,
                                               calendarPath = calendarPath)
  ...
  # 6.4 brush用户投资人数比例预测
  brush_type_num_ratio_forecast<-cash_flow_type_forecast(brush_type_num_ratio_train,
                                                     forecastDaySpan = forecastDaySpan,
                                                     calendarPath = calendarPath)
  # 6.5 brush用户单人投资额度预测
  brush_amount_per_num_forecast<-cash_flow_type_forecast(brush_amount_per_num_train,
                                                     forecastDaySpan = forecastDaySpan,
                                                     calendarPath = calendarPath)
  # 6.6 brush用户人数预测
  ...
  # 6.7 brush用户投资预测结果
  brush_result<-brush_type_num_ratio_forecast[brush_amount_per_num_forecast,
                                      on=c("date"="date"),nomatch=NA,mult="all"]%>%
    .[,c("date","total","i.total","non_res_total","i.non_res_total")]%>%
    .[brush_type_num_forecast,on=c("date"="date"),nomatch=NA,mult="all"]%>%
    .[,c("date","type","type_num","total","i.total","non_res_total","i.non_res_total")]
  
  colnames(brush_result)<-c("date",
                        "type",
                        "type_num",
                        "type_num_ratio",
                        "amount_per_num",
  ...
  brush_result[,":="(amount=type_num*type_num_ratio*amount_per_num,
                 pure_amount=type_num*pure_type_num_ratio*pure_amount_per_num)]
  
  # ******************************** 7. newcomer 用户投资预测**************************************************
  # 7.1 newcomer用户投资人数比例训练
  newcomer_type_num_ratio_train<-cash_flow_type_train(item = "type_num_ratio",
                                               user_type = "newcomer",
                                               trainTimeSpan = newcomer_type_trainTimeSpan,
                                               trainSetPath = trainSetPath,
                                               calendarPath = calendarPath)
  # 7.2 newcomer用户单人投资额度训练
  newcomer_amount_per_num_train<-cash_flow_type_train(item = "amount_per_num",
                                               user_type = "newcomer",
                                               trainTimeSpan = newcomer_type_trainTimeSpan,
                                               trainSetPath = trainSetPath,
                                               calendarPath = calendarPath)
  ...
  # 7.4 newcomer用户投资人数比例预测
  newcomer_type_num_ratio_forecast<-cash_flow_type_forecast(newcomer_type_num_ratio_train,
                                                     forecastDaySpan = forecastDaySpan,
                                                     calendarPath = calendarPath)
  # 7.5 newcomer用户单人投资额度预测
  newcomer_amount_per_num_forecast<-cash_flow_type_forecast(newcomer_amount_per_num_train,
                                                     forecastDaySpan = forecastDaySpan,
                                                     calendarPath = calendarPath)
  # 7.6 newcomer用户人数预测
  ...
  # 7.7 newcomer用户投资预测结果
  newcomer_result<-newcomer_type_num_ratio_forecast[newcomer_amount_per_num_forecast,
                                      on=c("date"="date"),nomatch=NA,mult="all"]%>%
    .[,c("date","total","i.total","non_res_total","i.non_res_total")]%>%
    .[newcomer_type_num_forecast,on=c("date"="date"),nomatch=NA,mult="all"]%>%
    .[,c("date","type","type_num","total","i.total","non_res_total","i.non_res_total")]
  
  colnames(newcomer_result)<-c("date",
                        "type",
                        "type_num",
                        "type_num_ratio",
                        "amount_per_num",
  ...
  newcomer_result[,":="(amount=type_num*type_num_ratio*amount_per_num,
                 pure_amount=type_num*pure_type_num_ratio*pure_amount_per_num)]
