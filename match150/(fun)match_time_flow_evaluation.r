  ...
  if(nrow(redeem_asset)>0){
	...
	for(m in 1:nrow(redeem_asset)){
	  ...
	  if(nrow(focus)==0){
        ...
        rest_redeem_amount<-ifelse(round(focus$unmatched_premium-redeem_asset_focus$amount,2)>=0,
                                   0,
                                   round(redeem_asset_focus$amount-focus$unmatched_premium,2))
		#--改变资金队列的相应记录
        cash_flow[userid==redeem_asset_focus$userid,
                  unmatched_premium:=round(unmatched_premium-(redeem_asset_focus$amount-rest_redeem_amount),2)]
		#--改变赎回资产信息的记录
