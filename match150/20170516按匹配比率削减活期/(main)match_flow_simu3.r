...
source('~/RStudio/match150/20170516按匹配比率削减活期/(fun)match_time_flow_evaluation2.r', encoding = 'UTF-8', echo=TRUE)
...
asset<-fread(paste0(path,"asset_simu_data.csv"))%>%
  .[,":="(create_time=as.POSIXct(create_time,format="%Y/%m/%d %H:%M"),
          end_time=as.POSIXct(end_time,format="%Y/%m/%d %H:%M"))]%T>%
...
regular<-user_list[unmatched_regular_premium>0,c("userid","unmatched_regular_premium","log_time")]%T>%
  setnames(.,"unmatched_regular_premium","unmatched_premium")%>%
  .[,unmatched_premium:=round(unmatched_premium,2)]

tPlus<-user_list[unmatched_tPlus_premium>0,c("userid","unmatched_tPlus_premium","log_time")]%T>%
  setnames(.,"unmatched_tPlus_premium","unmatched_premium")%>%
  .[,unmatched_premium:=round(unmatched_premium,2)]

current<-user_list[unmatched_current_premium>0,c("userid","unmatched_current_premium","log_time")]%T>%
...
target_ratio<-{
  b<-sum(current_now$unmatched_premium)+sum(tPlus_now$unmatched_premium)+sum(regular_now$unmatched_premium)
  ceiling(sum(asset_now$amount)/(b+sum(match_status_now$amount))*100)/100
}
############################## 1.时间流验证 #################################
...
target_ratio_log<-data.table()
end_for=length(time_seq)-1
for(index in 1:end_for){
  if(index>1){
    if((date(time_seq[index])-date(time_seq[index-1]))>0){
      target_ratio<-{
        b<-sum(current_now$unmatched_premium)+sum(tPlus_now$unmatched_premium)+sum(regular_now$unmatched_premium)
        ceiling(sum(asset_now$amount)/(b+sum(match_status_now$amount))*100)/100
      }
      print(paste("target ratio is set to",target_ratio,"!"))
    }
  }
  target_ratio_record<-data.table(index=index,
  ...
  target_ratio_log<-rbind(target_ratio_log,target_ratio_record)
}
