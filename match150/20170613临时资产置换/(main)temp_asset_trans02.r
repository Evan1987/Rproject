...
####################################################################################################
path0 = "F:/Project/20170603资产匹配穿透200人上线/20170613资产临时置换/20170714data/"
match_record<-fread(paste0(path0,"match_record.csv"))
asset_data<-fread(paste0(path0,"asset_data.csv"))
regular_info<-fread(paste0(path0,"regular_info.csv"))


fun.type<-function(x){
  switch(x,"1"="current","2"="tPlus","3"="regular")
}
exp_current_rate = 5
...
regularFocusWorse<-regularFocusBad[diff_rate<=-0.26,]


wait_up<-match_summary[userid%in%regularFocusWorse$userid,]
...
wanna_up<-match_record[userid%in%wait_up$userid & type==3,]%>%
  ...
  .[diff_rate< -0.5 & rate<6 & amount>=100,]%T>%
  setorder(.,diff_rate,-exp_rate,-amount)%>%
  .[,id:=.I]
...
rm(list=c("regular_summary","regular_info","regularFocus","regularFocusBad","match_summary"))
...
for(i in 1:adjust_num){
  ...
  adjust_temp<-head(temp,2)%>%{
    ...
    cbind(.[2,-c("id","amount","trans_no")]%T>%setnames(.,vars,str_c("down_",vars)),
          .[1,-"id"]%T>%setnames(.,vars,str_c("up_",vars)))
  }%>%.[,amount:=abs(amount)]
  ...
  match_record<-{
    rbindlist(list(match_record[isfix==0,][temp[amount<0,],
                                           on=c("asset_id"="asset_id",
                                                "userid"="userid",
                                                "type"="type"),
                                           amount:=round(amount+i.amount,2)],
                   temp[,-"trans_no"][amount>0,isfix:=1],
                   match_record[isfix==1,]),fill = T)
  }%>%.[amount>0,]
