...
################# 计算当前可匹资金和可匹资产 #################
now_time = Sys.time()
cash_vs_asset_result<-{
  x<-remain_cash[,.(amount=sum(amount)),by=account_type]
  y<-remain_asset[,-"avail_num"]
  cbind(x,y)%>%.[,log_time:=now_time]
}
...
for(i in 1:nlen){
  ...
  a = round(nrow(regular_seq)/(remain_asset$avail_num+num),4)%>%pmin(.,1)
  b = round(nrow(tPlus_seq)/(remain_asset$avail_num+num-nrow(regular_seq)),4)%>%pmin(.,1)
