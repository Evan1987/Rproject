...
source('~/rstudio/match150/new/(fun)fun.redeem_select_asset.r', encoding = 'UTF-8')
source('~/rstudio/match150/new/(fun)match_temp2.r', encoding = 'UTF-8', echo=TRUE)
source('~/rstudio/match150/new/(fun)match_time_flow_evaluation.r', encoding = 'UTF-8', echo=TRUE)
...
fun.invest<-function(unmatched_premium,log_time,now_time,remark){
  if(is.null(unmatched_premium)){
    ...
    remark_weight<-ifelse(is.na(remark),remark)*10
...
#----当前定期序列
regular_now<-copy(regular)%>%mutate(.,remark=as.integer(0))
#----当前T+N序列
tPlus_now<-copy(tPlus)%>%mutate(.,remark=as.integer(0))
#----当前活期序列
current_now<-copy(current)%>%mutate(.,remark=as.integer(0))
...
#----赎回冲抵锁定总表，先创建空的
redeem_frozen_now<-data.table()%>%tbl_df(.)
#----赎回资产总表，先创建空的
