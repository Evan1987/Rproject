...
# 一个目标方程: 总金额的最小值
fun.select_wise_slim<-function(exp_user_num,exp_num,exp_amount,target_data_summary){
  ...
  model$A<-matrix(c(rep(1,nrow(target_data_summary)),
                    ...
                    target_data_summary$amount),nrow = 3,byrow = T)
  # 约束方程等式关系
  model$sense<-c("=","=",">=")
  # 约束方程等式右边
  model$rhs = c(exp_user_num,exp_num,exp_amount)
  # 求解类型：0-1规划
  model$vtype = "B"
  # 目标优化方向
