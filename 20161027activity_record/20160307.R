...
#选取活动数据,并对数值格式进行处理。
mydata<-fread(files[6,1],encoding = "UTF-8",stringsAsFactors = F)
...
activity_span_days<-as.numeric(difftime("2016-03-16","2016-03-07"))+1
###投资时长列，为了计算之前定期投资频次
mydata[,span_days:=as.numeric(difftime("2016-03-07",reg_day,units="days"))]
###活动期投资频次
...
#去掉多余列
task[,(3:6):=NULL]
write.csv(task,"D:\\Project\\20161027用户活动标签\\csv\\02参与向量\\Result20160307.csv",row.names = F)