...
reg_to_invest1st<-read_csv(paste0(path,"reg_to_invest_data.csv"),locale = locale(encoding = "GBK"))%>%
  mutate(.,wait_time=ifelse(is.na(invest1st_time),Inf,difftime(invest1st_time,reg_time,units="days")%>%as.numeric(.)))%>%
  as.data.table(.)
quantile(reg_to_invest1st[!is.na(invest1st_time),]$wait_time,probs = seq(0,1,0.1))

valid_data<-reg_to_invest1st[(!is.na(invest1st_time))&wait_time>0,]
